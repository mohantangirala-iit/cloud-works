...
LTVAR=$(aws ec2 create-launch-template --launch-template-name ${12} --version-description AutoScalingVersion1 --launch-template-data file://config.json --region ${17} --output=text --query 'LaunchTemplate.LaunchTemplateId')
echo "Launch Template created..."

# Launch Template Id
LAUNCHTEMPLATEID=$LTVAR
...
ELBARN=$(aws elbv2 create-load-balancer --subnets $SUBNET2A $SUBNET2B --name $9 --security-groups $4 --query "LoadBalancers[*].LoadBalancerArn" --output=text)
echo $ELBARN
...
aws s3api create-bucket --bucket my-bucket ${19} --region ${17}
echo "Created S3 bucket: ${19}..."

echo "Creating S3 bucket: ${20}..."
aws s3api create-bucket --bucket my-bucket ${20} --region ${17}
...
URL=$(aws elbv2 describe-load-balancers --load-balancer-arns $ELBARN --output=text --query 'LoadBalancers[*].DNSName')
echo $URL
