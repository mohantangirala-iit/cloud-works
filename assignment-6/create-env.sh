...
if [ $# = 0 ]
    ...
    echo "******************************************************************************"
    DBIID=$(aws rds create-db-instance --db-instance-identifier ${22}  --db-instance-class db.t3.micro --engine --master-username $USERVALUE --master-user-password $PASSVALUE --allocated-storage 20 --db-name employee_database --tags="Key=assessment,Value=${7}" --output=text --query 'DBInstance.DBInstanceIdentifier')
    ...
    echo "******************************************************************************"
    echo "Waiting for RDS instance: to be created... $DBIID"
    echo "This might take around 5-15 minutes..."
    echo "******************************************************************************"
    aws rds wait db-instance-available --db-instance-identifier $DBIID
    echo "******************************************************************************"
    ...
    echo "******************************************************************************"
    # Append "-read-replica" to the ${22} to create the read-replica name
    DBRPID=$(aws rds create-db-instance-read-replica --db-instance-identifier "${22}-read-replica"   --source-db-instance-identifier $DBIID --tags="Key=assessment,Value=${7}" --output=text --query 'DBInstance.ReadReplicaSourceDBInstanceIdentifier')
    ...
    echo "******************************************************************************"
    aws rds wait db-instance-available --db-instance-identifier $DBRPID
    ...
    RDS_Address=$(aws rds describe-db-instances --db-instance-identifier ${22} --query "DBInstances[*].DBInstanceIdentifier")
    echo $RDS_Address
    echo "Retrieving the RDS Read Replica Endpoint Address and printing to the screen..."
    RDS_RR_Address=$(aws rds describe-db-instances --db-instance-identifier $DBRPID  --query "DBInstances[*].DBInstanceIdentifier")
    echo $RDS_RR_Address
# End of main if
fi
