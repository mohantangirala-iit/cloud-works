...
TARGETARN=$(aws elbv2 describe-target-groups --name $8 --query "TargetGroups[*].TargetGroupArn" --output=text)
echo $TARGETARN
...
ELBARN=$(aws elbv2 describe-load-balancers --names $9 --output=text --query "LoadBalancers[*].LoadBalancerArn")
echo $ELBARN
