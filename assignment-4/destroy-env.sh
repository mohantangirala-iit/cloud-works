...
ASGNAMES=$(aws autoscaling describe-auto-scaling-groups --output=text --query 'AutoScalingGroups[*].AutoScalingGroupName' )
if [ "$ASGNAMES" != "" ]
  then
...
TARGETARN=$(aws elbv2 describe-target-groups --names mt-tg --query 'TargetGroups[*].TargetGroupArn' --output=text)
if [ "$TARGETARN" != "" ]
  then
...
ELBARN=$(aws elbv2 describe-load-balancers --names mt-elb --output=text --query "LoadBalancers[*].LoadBalancerArn")
echo $ELBARN
